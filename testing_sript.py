...
# portfolio.to_excel('problem_portfolio.xlsx', index=False)  # index=False to exclude row numbers
# #portfolio = pd.read_excel('portfolio.xlsx', sheet_name='Sheet1')
portfolio = pd.read_csv('problem_portfolio.csv')
# portfolio = pd.read_excel('problem_portfolio.xlsx',sheet_name='Sheet1')
...
# Calculate capital gains and taxes
        ...
        model.addConstr(
            ...
            gp.quicksum(buy_amounts[t] for t in portfolio['Ticker'].unique())  >= 0.97*initial_value,"maintain_total_value")

        model.addConstr(
            gp.quicksum((1 - sell_fractions[i]) * portfolio.loc[i, 'Value'] for i in portfolio.index) +
            gp.quicksum(buy_amounts[t] for t in portfolio['Ticker'].unique())  <= initial_value,"maintain_total_value_2")
        ...
        if model.status == GRB.OPTIMAL:
            ...
            print(f"Tax paid: ${actual_tax_paid:.2f}")
            ...
            new_stock_value = optimized_portfolio[optimized_portfolio['Ticker'].str.contains('STOCK')]['Final Value'].sum()
            ...
            print(f"New stock value: ${new_stock_value:.2f}")
            print(f"New total value: ${new_total_value:.2f}")
            print(f"New stock allocation: {new_stock_allocation:.2%}")
            print(f"New bond allocation: {new_bond_allocation:.2%}")

            total_tax_paid = optimized_portfolio['Tax Paid'].sum()
            print(f"Total tax paid: ${total_tax_paid:.2f}")

            print("Buy Summary")
            print(buy_results[buy_results['Buy Quantity'] > 0])

            print("Sell Summary")
            print(optimized_portfolio[optimized_portfolio['Sell Quantity'] > 0][
...
optimized_portfolio.to_excel(f'opt_port_{current_datetime}.xlsx', index=False)